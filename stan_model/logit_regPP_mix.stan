data {
  ...
  real w_mix;             // mixture weight
  real int_dem;           // normalization constant
}
parameters {
  real beta; 
}
...
model {
  ...
  p0[i] = 1 / (1 + exp(-z0[i]));
  lik_PP[i] =  bernoulli_lpmf(y0[i] | p0[i]);
  }
  lik_final = alpha0*sum(lik_PP) + normal_lpdf(beta | 0, sqrt(1.34)) - log(int_dem);
  target += log_mix( w_mix, lik_final, normal_lpdf(beta | 0, sqrt(1.34)));
}
